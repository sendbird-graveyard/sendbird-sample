...
class FileMessageTableViewCell: UITableViewCell {
    ...
    private func initViews() {
        ...
        contentView.addSubview(self.fileImageView!)
    ...
    func setModel(model: SendBirdFileLink) {
        ...
        self.filesizeLabel?.text = String.init(format: "%.2fMB", (CFloat(fileSize) / 1024.0 / 1024.0))
    }
    
    func getHeightOfViewCell(totalWidth: CGFloat) -> CGFloat {
        let nameRect: CGRect? = self.nicknameLabel?.attributedText?.boundingRectWithSize(CGSizeMake(kFileMessageNameWidth, CGFloat.max), options: NSStringDrawingOptions.UsesLineFragmentOrigin, context: nil)
